package com.ultrabytecoder.kryptakeep.providers.ton.wallet

import com.ultrabytecoder.kryptakeep.providers.ton.address.TonAddress
import com.ultrabytecoder.kryptakeep.providers.ton.address.contractAddress
import com.ultrabytecoder.kryptakeep.providers.ton.boc.*
import com.ultrabytecoder.kryptakeep.providers.ton.types.MessageRelaxed

abstract class WalletContract(
    val workchain: Int,
    val publicKey: ByteArray,
    val walletId: Int,
    val code: Cell
) {
    val data: Cell = buildDataCell()
    val address: TonAddress = contractAddress(workchain, code, data)

    protected open fun buildDataCell(): Cell = beginCell()
        .storeUint(0, 32)
        .storeUint(walletId.toLong(), 32)
        .storeBuffer(publicKey)
        .endCell()

    abstract fun createTransfer(
        seqno: Int,
        secretKey: ByteArray,
        messages: List<MessageRelaxed>,
        sendMode: Int,
        timeout: Int?
    ): Cell
}

class WalletContractV3R2 private constructor(
    workchain: Int,
    publicKey: ByteArray,
    walletId: Int
) : WalletContract(workchain, publicKey, walletId, CODE) {

    companion object {
        private val CODE: Cell = Cell.fromHex(
            ""
        )

        fun create(workchain: Int = 0, publicKey: ByteArray, walletId: Int? = null): WalletContractV3R2 =
            WalletContractV3R2(workchain, publicKey, walletId ?: (698983191 + workchain))
    }

    override fun createTransfer(
        seqno: Int,
        secretKey: ByteArray,
        messages: List<MessageRelaxed>,
        sendMode: Int,
        timeout: Int?
    ): Cell = createWalletTransferV3(seqno, walletId, secretKey, messages, sendMode, timeout)
}

class WalletContractV4 private constructor(
    workchain: Int,
    publicKey: ByteArray,
    walletId: Int
) : WalletContract(workchain, publicKey, walletId, CODE) {

    companion object {
        private val CODE: Cell = Cell.fromHex(
            ""
        )

        fun create(workchain: Int = 0, publicKey: ByteArray, walletId: Int? = null): WalletContractV4 =
            WalletContractV4(workchain, publicKey, walletId ?: (698983191 + workchain))
    }

    override fun buildDataCell(): Cell = beginCell()
        .storeUint(0, 32)
        .storeUint(walletId.toLong(), 32)
        .storeBuffer(publicKey)
        .storeBit(false) // empty plugins dict
        .endCell()

    override fun createTransfer(
        seqno: Int,
        secretKey: ByteArray,
        messages: List<MessageRelaxed>,
        sendMode: Int,
        timeout: Int?
    ): Cell = createWalletTransferV4(seqno, walletId, secretKey, messages, sendMode, timeout)
}
