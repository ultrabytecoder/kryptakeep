package com.ultrabytecoder.kryptakeep.providers.ton.boc

import kotlin.io.encoding.Base64
import kotlin.io.encoding.ExperimentalEncodingApi
import kotlin.test.Test
import kotlin.test.assertEquals
import kotlin.test.assertTrue

@OptIn(ExperimentalEncodingApi::class)
class BocSerializationTest {

    private val wallets = listOf(
        "B5EE9C72410101010044000084FF0020DDA4F260810200D71820D70B1FED44D0D31FD3FFD15112BAF2A122F901541044F910F2A2F80001D31F3120D74A96D307D402FB00DED1A4C8CB1FCBFFC9ED5441FDF089",
        "B5EE9C724101010100530000A2FF0020DD2082014C97BA9730ED44D0D70B1FE0A4F260810200D71820D70B1FED44D0D31FD3FFD15112BAF2A122F901541044F910F2A2F80001D31F3120D74A96D307D402FB00DED1A4C8CB1FCBFFC9ED54D0E2786F",
        "B5EE9C7241010101005F0000BAFF0020DD2082014C97BA218201339CBAB19C71B0ED44D0D31FD70BFFE304E0A4F260810200D71820D70B1FED44D0D31FD3FFD15112BAF2A122F901541044F910F2A2F80001D31F3120D74A96D307D402FB00DED1A4C8CB1FCBFFC9ED54B5B86E42",
        "B5EE9C724101010100570000AAFF0020DD2082014C97BA9730ED44D0D70B1FE0A4F2608308D71820D31FD31F01F823BBF263ED44D0D31FD3FFD15131BAF2A103F901541042F910F2A2F800029320D74A96D307D402FB00E8D1A4C8CB1FCBFFC9ED54A1370BB6",
        "B5EE9C724101010100630000C2FF0020DD2082014C97BA218201339CBAB19C71B0ED44D0D31FD70BFFE304E0A4F2608308D71820D31FD31F01F823BBF263ED44D0D31FD3FFD15131BAF2A103F901541042F910F2A2F800029320D74A96D307D402FB00E8D1A4C8CB1FCBFFC9ED54044CD7A1",
        "B5EE9C724101010100620000C0FF0020DD2082014C97BA9730ED44D0D70B1FE0A4F2608308D71820D31FD31FD31FF82313BBF263ED44D0D31FD31FD3FFD15132BAF2A15144BAF2A204F901541055F910F2A3F8009320D74A96D307D402FB00E8D101A4C8CB1FCB1FCBFFC9ED543FBE6EE0",
        ""
    )

    private fun ByteArray.toHexString(): String = joinToString("") { "%02x".format(it) }

    private fun String.decodeHex(): ByteArray =
        chunked(2).map { it.toInt(16).toByte() }.toByteArray()

    private fun String.decodeBase64(): ByteArray = Base64.decode(this)

    private fun ByteArray.encodeBase64(): String = Base64.encode(this)

    private fun cellToString(cell: Cell): String {
        val sb = StringBuilder("x{${cell.bits.toString()}}")
        for (ref in cell.refs) {
            sb.append("\n ")
            sb.append(cellToString(ref))
        }
        return sb.toString()
    }

    private fun assertCellEquals(expected: Cell, actual: Cell, message: String = "") {
        assertTrue(
            expected.hash().contentEquals(actual.hash()),
            buildString {
                if (message.isNotEmpty()) append("$message: ")
                append("Cell hashes don't match")
            }
        )
    }

    @Test
    fun shouldParseWalletCode() {
        for (w in wallets) {
            val c = Cell.fromHex(w)
            val b = c.toBoc(idx = false, crc32 = true)
            val c2 = Cell.fromBoc(b)[0]
            assertCellEquals(c, c2)
        }
    }

    @Test
    fun shouldSerializeSingleCellWithEmptyBits() {
        val cell = beginCell().endCell()

        assertEquals("x{}", cellToString(cell))
        assertEquals("lqKW0iTyhcZ77pPDD4owkVfw2qNdxbh+QQt4YwoJz8c=", cell.hash().encodeBase64())

        assertEquals("te6ccgEBAQEAAgAAAA==", cell.toBoc(idx = false, crc32 = false).encodeBase64())
        assertEquals("te6cckEBAQEAAgAAAEysuc0=", cell.toBoc(idx = false, crc32 = true).encodeBase64())
        assertEquals("te6ccoEBAQEAAgAAAAA=", cell.toBoc(idx = true, crc32 = false).encodeBase64())
        assertEquals("te6ccsEBAQEAAgAAAAC1U5ck", cell.toBoc(idx = true, crc32 = true).encodeBase64())

        assertCellEquals(cell, Cell.fromBoc("te6ccgEBAQEAAgAAAA==".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6cckEBAQEAAgAAAEysuc0=".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccoEBAQEAAgAAAAA=".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccsEBAQEAAgAAAAC1U5ck".decodeBase64())[0])
    }

    @Test
    fun shouldSerializeSingleCellWithByteAlignedBits() {
        val cell = beginCell().storeUint(123456789, 32).endCell()

        assertEquals("x{075BCD15}", cellToString(cell))
        assertEquals("keNT38owvINaYYHwYjE1R8HYk0c1NSMH72u+/aMJ+1c=", cell.hash().encodeBase64())

        assertEquals("te6ccgEBAQEABgAACAdbzRU=", cell.toBoc(idx = false, crc32 = false).encodeBase64())
        assertEquals("te6cckEBAQEABgAACAdbzRVRblCS", cell.toBoc(idx = false, crc32 = true).encodeBase64())
        assertEquals("te6ccoEBAQEABgAAAAgHW80V", cell.toBoc(idx = true, crc32 = false).encodeBase64())
        assertEquals("te6ccsEBAQEABgAAAAgHW80Vyf0TAA==", cell.toBoc(idx = true, crc32 = true).encodeBase64())

        assertCellEquals(cell, Cell.fromBoc("te6ccgEBAQEABgAACAdbzRU=".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6cckEBAQEABgAACAdbzRVRblCS".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccoEBAQEABgAAAAgHW80V".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccsEBAQEABgAAAAgHW80Vyf0TAA==".decodeBase64())[0])
    }

    @Test
    fun shouldSerializeSingleCellWithNonAlignedBits() {
        val cell = beginCell().storeUint(123456789, 34).endCell()

        assertEquals("x{01D6F3456_}", cellToString(cell))
        assertEquals("Rk+nt8kkAyN9S1v4H0zwFbGs2INwpMHvESvPQbrI6d0=", cell.hash().encodeBase64())

        assertEquals("te6ccgEBAQEABwAACQHW80Vg", cell.toBoc(idx = false, crc32 = false).encodeBase64())
        assertEquals("te6cckEBAQEABwAACQHW80Vgb11ZoQ==", cell.toBoc(idx = false, crc32 = true).encodeBase64())
        assertEquals("te6ccoEBAQEABwAAAAkB1vNFYA==", cell.toBoc(idx = true, crc32 = false).encodeBase64())
        assertEquals("te6ccsEBAQEABwAAAAkB1vNFYMkX0oY=", cell.toBoc(idx = true, crc32 = true).encodeBase64())

        assertCellEquals(cell, Cell.fromBoc("te6ccgEBAQEABwAACQHW80Vg".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6cckEBAQEABwAACQHW80Vgb11ZoQ==".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccoEBAQEABwAAAAkB1vNFYA==".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccsEBAQEABwAAAAkB1vNFYMkX0oY=".decodeBase64())[0])
    }

    @Test
    fun shouldSerializeSingleCellWithSingleReference() {
        val refCell = beginCell().storeUint(123456789, 32).endCell()
        val cell = beginCell()
            .storeUint(987654321, 32)
            .storeRef(refCell)
            .endCell()

        assertEquals("x{3ADE68B1}\n x{075BCD15}", cellToString(cell))
        assertEquals("goaQYcsXO2c/gd3qvMo3ncEjzpbU7urNQ7hPDo0qC1c=", cell.hash().encodeBase64())

        assertEquals("te6ccgEBAgEADQABCDreaLEBAAgHW80V", cell.toBoc(idx = false, crc32 = false).encodeBase64())
        assertEquals("te6cckEBAgEADQABCDreaLEBAAgHW80VSW/75w==", cell.toBoc(idx = false, crc32 = true).encodeBase64())
        assertEquals("te6ccoEBAgEADQAABwEIOt5osQEACAdbzRU=", cell.toBoc(idx = true, crc32 = false).encodeBase64())
        assertEquals("te6ccsEBAgEADQAABwEIOt5osQEACAdbzRWZVy8t", cell.toBoc(idx = true, crc32 = true).encodeBase64())

        assertCellEquals(cell, Cell.fromBoc("te6ccgEBAgEADQABCDreaLEBAAgHW80V".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6cckEBAgEADQABCDreaLEBAAgHW80VSW/75w==".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccoEBAgEADQAABwEIOt5osQEACAdbzRU=".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccsEBAgEADQAABwEIOt5osQEACAdbzRWZVy8t".decodeBase64())[0])
    }

    @Test
    fun shouldSerializeSingleCellWithMultipleReferences() {
        val refCell = beginCell().storeUint(123456789, 32).endCell()
        val cell = beginCell()
            .storeUint(987654321, 32)
            .storeRef(refCell)
            .storeRef(refCell)
            .storeRef(refCell)
            .endCell()

        assertEquals(
            "x{3ADE68B1}\n x{075BCD15}\n x{075BCD15}\n x{075BCD15}",
            cellToString(cell)
        )
        assertEquals("cks0wbfqFZE9/yb0sWMWQGoj0XBOLkUi+aX5xpJ6jjA=", cell.hash().encodeBase64())

        assertEquals("te6ccgEBAgEADwADCDreaLEBAQEACAdbzRU=", cell.toBoc(idx = false, crc32 = false).encodeBase64())
        assertEquals("te6cckEBAgEADwADCDreaLEBAQEACAdbzRWpQD2p", cell.toBoc(idx = false, crc32 = true).encodeBase64())
        assertEquals("te6ccoEBAgEADwAACQMIOt5osQEBAQAIB1vNFQ==", cell.toBoc(idx = true, crc32 = false).encodeBase64())
        assertEquals("te6ccsEBAgEADwAACQMIOt5osQEBAQAIB1vNFT/vUE4=", cell.toBoc(idx = true, crc32 = true).encodeBase64())

        assertCellEquals(cell, Cell.fromBoc("te6ccgEBAgEADwADCDreaLEBAQEACAdbzRU=".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6cckEBAgEADwADCDreaLEBAQEACAdbzRWpQD2p".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccoEBAgEADwAACQMIOt5osQEBAQAIB1vNFQ==".decodeBase64())[0])
        assertCellEquals(cell, Cell.fromBoc("te6ccsEBAgEADwAACQMIOt5osQEBAQAIB1vNFT/vUE4=".decodeBase64())[0])
    }

    @Test
    fun shouldHashTxWithMerkleBody() {
        val bocHex = ""
        val expectedHash = "ca676f0f30d21c8828d1094424797085b603e991d26943ee17ee5d77ac4b0896"

        val cell = Cell.fromBoc(bocHex.decodeHex())[0]
        assertEquals(expectedHash, cell.hash().toHexString())
    }

    @Test
    fun shouldDeserializeAndSerializeLibraryCell() {
        val bocBase64 = "te6ccgEBAgEALQABDv8AiNDtHtgBCEICGbgzd5nhZ9WhSM+4juFCvgMYJOtxthFdtTKIH6M/6SM="
        val cell = Cell.fromBoc(bocBase64.decodeBase64())[0]
        val serialized = cell.toBoc(idx = false, crc32 = false).encodeBase64()
        assertEquals(bocBase64, serialized)
    }

    @Test
    fun shouldParseAccountStateTestRoundtrip() {
        val bocBase64 = "te6ccgEBBAEArwAJRgPIr248LcbQSSCsDD5Rb27WLhRGYiTEGG+uChgAAXoNHAAIASJxwAtrH/x8t+GjDO5/X/f1fk4Rw3oYx+9S1gRE8vya04qzwiyFkEMdYglgAAAaNN8fbBluIJfFw9NAAgMoSAEB/rX/aCDi/w2Ug+fg1iyBfYRniftK5YDIeIZtlZ2r1cAAByhIAQEg0z54hgTX/ohMEnHs6qluCydagWgxQoxSyLwK8qfAOQAA"
        val boc = bocBase64.decodeBase64()
        val c = Cell.fromBoc(boc)[0]
        val b = c.toBoc(idx = false, crc32 = true)
        val c2 = Cell.fromBoc(b)[0]
        assertCellEquals(c, c2)
    }

    @Test
    fun shouldParseAccountStateTestPrunedRoundtrip() {
        val bocBase64 = "te6ccgEBBAEArwAJRgPIr248LcbQSSCsDD5Rb27WLhRGYiTEGG+uChgAAXoNHAAIASJxwAtrH/x8t+GjDO5/X/f1fk4Rw3oYx+9S1gRE8vya04qzwiyFkEMdYglgAAAaNN8fbBluIJfFw9NAAgMoSAEB/rX/aCDi/w2Ug+fg1iyBfYRniftK5YDIeIZtlZ2r1cAAByhIAQEg0z54hgTX/ohMEnHs6qluCydagWgxQoxSyLwK8qfAOQAA"
        val boc = bocBase64.decodeBase64()
        val c = Cell.fromBoc(boc)[0]
        val b = c.toBoc(idx = false, crc32 = true)
        val c2 = Cell.fromBoc(b)[0]
        assertCellEquals(c, c2)
    }

    @Test
    fun shouldDeserializeBlock() {
        val shardAccount = beginCell().storeUint(1, 32).endCell()
        val extra = beginCell().storeUint(2, 32).endCell()
        val root = beginCell()
            .storeUint(0, 32)
            .storeRef(shardAccount)
            .storeRef(extra)
            .endCell()

        val serialized = root.toBoc(idx = false, crc32 = false)
        val deserialized = Cell.fromBoc(serialized)

        assertEquals(1, deserialized.size)
        assertCellEquals(root, deserialized[0])
    }

    // From ton-core JS reference: boc/cell/__testdata__/block.txt
    private val blockBocBase64 = ""

    @Test
    fun shouldDeserializeReferenceBlock() {
        val cells = Cell.fromBoc(blockBocBase64.decodeBase64())
        assertTrue(cells.isNotEmpty())
    }
}
